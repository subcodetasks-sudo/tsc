...
import { buildPartnerFormData, mapPartnerToFormDefaults } from "@/features/admin/lib/partner-form-utils"
...
export function AdminPartnerEditForm({ partner, locale }: { partner: any; locale: string }) {
  ...
  const {
    ...
    defaultValues: mapPartnerToFormDefaults(partner, locale),
  })

  useEffect(() => {
    reset(mapPartnerToFormDefaults(partner, locale))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [partner?.id, locale])

  const logoFile = watch("logoFile")
  const logoPreview = watch("logoPreview")
  const existingLogo = watch("existingLogo")

  const onSubmit = handleSubmit((values, e) => {
    e?.preventDefault()
    setError(null)
    setSuccess(false)
    const formData = buildPartnerFormData(values, partner.id)

    startTransition(async () => {
      const result = await savePartnerAction(formData, locale, partner?.id)
      if (!result.ok) {
  ...
  const imageSrc = logoPreview || (existingLogo ? resolveImageUrl(existingLogo) : null)
