...
import { buildSuccessStoryFormData, mapStoryToFormDefaults } from "@/features/admin/lib/success-story-form-utils"
import { AdminLocaleTextField } from "./admin-locale-text-field"
import { AdminImageUploadField } from "./admin-image-upload-field"

export function AdminSuccessStoryEditForm({ story, locale }: { story: any; locale: string }) {
  ...
  const {
    ...
    defaultValues: mapStoryToFormDefaults(story, locale),
  })

  // Reset the form when navigating client-side between different stories.
  useEffect(() => {
    reset(mapStoryToFormDefaults(story, locale))
  ...
  const onSubmit = handleSubmit((values, e) => {
    ...
    setError(null)
    setSuccess(false)
    const formData = buildSuccessStoryFormData(values, story.id)

    startTransition(async () => {
      const result = await saveSuccessStoryAction(formData, locale, story?.id )
      if (!result.ok) {
