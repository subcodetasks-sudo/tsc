...
export function AdminServiceEditForm({
  ...
  const {
    ...
    defaultValues: mapServiceToFormDefaults(service, locale),
  })

  // Reset the form when navigating client-side between different services.
  useEffect(() => {
    reset(mapServiceToFormDefaults(service, locale))
  ...
  const existingIcons = watch("existingIcons")
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <AdminImageUploadField
        title={t("serviceIcon")}
        locale={editLocale}
        imageSrc={iconPreviews?.[editLocale] || existingIcons?.[editLocale] || null}
