...
import { buildNewsFormData, mapNewsToFormDefaults } from "@/features/admin/lib/news-form-utils"
import { AdminLocaleTextField } from "./admin-locale-text-field"
import { AdminImageUploadField } from "./admin-image-upload-field"

export function AdminNewsEditForm({ newsItem, locale }: { newsItem: any; locale: string }) {
  ...
  const {
    ...
    defaultValues: mapNewsToFormDefaults(newsItem, locale),
  })

  // Reset the form when navigating client-side between different news items.
  useEffect(() => {
    reset(mapNewsToFormDefaults(newsItem, locale))
  ...
  const onSubmit = handleSubmit((values) => {
    setError(null)
    setSuccess(false)
    const formData = buildNewsFormData(values, newsItem.id)

    startTransition(async () => {
      const result = await saveNewsAction(formData, locale, newsItem.id)
      if (!result.ok) {
